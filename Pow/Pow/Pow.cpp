...
Associativity Pow::getAssociativity() const {
	return c_associativity;
}

bool Pow::IsThatOp(const std::string& s) const {
	if (s == "^") {
		return true;
	}
	return false;
...
ERROR_ Pow::count(const std::string& expression, double& answer) {
